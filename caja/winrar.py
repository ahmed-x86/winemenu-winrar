...
class WinRARMenuProvider(GObject.GObject, Caja.MenuProvider):
    def __init__(self):
        super().__init__()
    ...
    def get_file_items(self, *args):
        ...
        main_item = Caja.MenuItem(name='WineMenu::WinRAR', label='WinRAR')
        main_submenu = Caja.Menu()
        ...
        for action in compress_actions:
            item = Caja.MenuItem(name=f"WinRARAction::{action['id']}", label=action['name'])
    ...
    def execute_wine_app(self, menu, files, action_id):
        ...
        if action_id.startswith('COMPRESS'):
            first_file = files[0]
            working_dir = os.path.dirname(first_file.get_location().get_path())
        ...
        for file in files_to_process:
            ...
            if not linux_filepath: continue
